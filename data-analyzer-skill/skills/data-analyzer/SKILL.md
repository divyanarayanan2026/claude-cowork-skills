...
name: data-analyzer
description: >
  **Data Analyzer**: Given a CSV, Excel (.xlsx/.xls/.xlsm), or TSV data file,
  produce a complete analysis with charts, KPI cards, key findings, summary
  statistics, categorical breakdowns, and a data quality report — all delivered
  as a polished dark-mode HTML report the user can open and share.

  ALWAYS use this skill when the user has a data file and wants insight from it
  — even if they phrase it casually. Trigger for: "analyze my data / CSV /
  spreadsheet", "what's in this file?", "make me some charts from this",
  "show me trends", "my boss wants a summary of this", "find patterns in this
  data", "what's going on with these numbers?", "pull insights from this
  dataset". Also trigger when the user uploads a data file and asks anything
  exploratory about it. Do NOT trigger when the user wants to: clean or reformat
  a file without analysis (use xlsx skill), read a PDF, build a live dashboard
  in code, write a data processing script, or simply explain a statistical
  concept without a file attached.
---
